...
-- first: create table. this happens OUTSIDE of the SP because we only want to do this once
CREATE TABLE ArchiveRegistration (OfferingCode INT, StudentID INT, Mark DECIMAL(5, 2), WithdrawYN CHAR(1))

GO

CREATE PROCEDURE ArchiveRegistrationSP (@Year CHAR(4)) AS

-- check params. if missing, RAISERROR
-- otherwise: 
	-- BEGIN TRANSACTION
	-- copy records into ArchiveRegistration (INSERT / SELECT)
	-- check if INSERT worked. if not, RAISERROR & ROLLBACK TRANSACTION
	-- otherwise: DELETE FROM Registration
		-- check if DELETE worked. if not, RAISERROR & ROLLBACK TRANSACTION
