...
CREATE TRIGGER TR_Course_Update_CourseCost
ON Course
FOR UPDATE
AS
...
IF @@ROWCOUNT > 0 AND UPDATE(CourseCost) -- run trigger only if CourseCost was UPDATEd
	BEGIN
	IF EXISTS ( -- are there records where the new value is MORE than 20% greater than the old value?
		SELECT * FROM inserted
		INNER JOIN deleted ON inserted.CourseId = deleted.CourseID
		WHERE inserted.CourseCost > 1.20 * deleted.CourseCost
	)
		BEGIN
		ROLLBACK TRANSACTION
		RAISERROR('course cost cannot be increased by more than 20 percent', 16, 1)
		END
