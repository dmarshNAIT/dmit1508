...
AS
SELECT CourseName --, COUNT(StudentID) AS NumberStudents
FROM Course
INNER JOIN Offering ON Course.CourseId = Offering.CourseID
INNER JOIN Registration ON Registration.OfferingCode = Offering.OfferingCode
GROUP BY Course.CourseId, CourseName
...
-- ALTERNATES:
CREATE VIEW NumStudentsPerCourse
AS
SELECT CourseName, COUNT(StudentID) AS NumberStudents
FROM Course
INNER JOIN Offering ON Course.CourseId = Offering.CourseID
...
SELECT CourseName
FROM NumStudentsPerCourse
WHERE NumberStudents = (SELECT MIN(NumberStudents) AS MinStudents FROM NumStudentsPerCourse)
...
FROM (
	...
	INNER JOIN Registration ON Registration.OfferingCode = Offering.OfferingCode
	GROUP BY Course.CourseId, CourseName
) AS InnerQuery -- giving a name to the subquery to make SQL happy
WHERE NumberStudents = (SELECT MIN(NumberStudents) AS MinStudents FROM NumStudentsPerCourse)
...
AS
SELECT DISTINCT FirstName + ' ' + LastName AS FullName, PaymentTypeDescription
FROM Student
